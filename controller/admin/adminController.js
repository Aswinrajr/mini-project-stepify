  ...
  try {
    ...
    if (existAdmin) {
      res.render("adminSignUpPage", {
        msg: "Admin alredy exist please Log In",
      });
    } else {
    ...
    if (adminData) {
      ...
      if (matchPassword) {
        ...
        res.render("adminLoginPage", {
          msg: "Your Password is Incorrect Please Enter Registered Password",
        });
      }
    } else {
      ...
      res.render("adminLoginPage", {
        msg: "Please enter the registered Email and Password",
      });
    }
  } catch (err) {
...
const downloadSalesReport = async (req, res) => {
  try {
    ...
    console.log(
      `Orders for report from ${startDate} to ${endDate} is: `,
      orders
    );
...
const viewUserProfile = async (req, res) => {
  try {
    ...
    deliveredData = await Order.find({
      userId: id,
      "items.status": "Delivered",
    }).count();
    canceledData = await Order.find({
      userId: id,
      "items.status": "Cancel",
    }).count();
    returnedData = await Order.find({
      userId: id,
      "items.status": "Return",
    }).count();
