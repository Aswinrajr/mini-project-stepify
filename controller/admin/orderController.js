...
const viewOrders = async (req, res) => {
  try {
    if (req.session.admin_id) {
      ...
      const productIds = viewOrders.items.map((item) =>
...
const outForDelivery = async (req, res) => {
  try {
    if (req.session.admin_id) {
      ...
      await Order.updateOne(
        { _id: orderId, "items.ProductId": productId },
        { $set: { "items.$.status": "Out for Delivery" } }
      );
      res.redirect(`/admin/order/${orderId}`);
    } else {
      res.redirect("/");
...
const cancelOrder = async (req, res) => {
  try {
    if (req.session.admin_id) {
      ...
      await Order.updateOne(
        { _id: orderId, "items.ProductId": productId },
        { $set: { "items.$.status": "Cancel" } }
      );
      console.log("Payment method: ", viewOrders.paymentMethod);
      const coupon = await Coupon.findOne({
        couponCode: viewOrders.usedCouponCode,
      });
      let couponAmount;
      if (coupon) {
      ...
      if (viewOrders.paymentMethod === "Online Payment") {
        ...
        const refundOrder = viewOrders.items.find(
          (item) => item.ProductId == productId
        );
        if (coupon && viewOrders.refund === false) {
          console.log(
            "Money added to the wallet deducting the coupon amount: ",
            refundOrder
          );
